...
bool MemoryAutomaton<T>::accepts() {
    ...
    return m_Pos == m_Tape.size() && Automaton<T>::m_FinalStates.find(Automaton<T>::m_CurState) != Automaton<T>::m_FinalStates.end();
}
...
void MemoryAutomaton<T>::execTransition(std::string s, T state) {
    if (s=="?"){
        ...
        m_Memory[getMemory(Automaton<T>::m_CurState, s[1])].first = true;
        m_Memory[getMemory(Automaton<T>::m_CurState, s[1])].second = "";
        Automaton<T>::m_CurState=state;
    }
    else if (s[0]==']'){
        m_Memory[getMemory(Automaton<T>::m_CurState, s[1])].first = false;
        Automaton<T>::m_CurState=state;
    }
    else if (isupper(s[0])){
        bool equal = true;
        if (m_Pos+m_Memory[getMemory(Automaton<T>::m_CurState, s[0])].second.size() <= m_Tape.size() && !m_Memory[getMemory(Automaton<T>::m_CurState, s[0])].first) {
        ...
        if (equal)
        {
            for (int i = 0; i < m_Memory.size(); i++){
                if (m_Memory[i].first) {
                    m_Memory[i].second = m_Memory[i].second+m_Memory[getMemory(Automaton<T>::m_CurState, s[0])].second;
                }
            }
...
void MemoryAutomaton<T>::print() {
    ...
    for (auto it = ++Automaton<T>::m_FinalStates.begin(); it != Automaton<T>::m_FinalStates.end(); it++) std::cout << "," << *it;
    std::cout << "})" << std::endl;
